const COLOR_STOP_RE = /(#[0-9A-Fa-f]{3,8}|rgba?\([^)]+\)|hsla?\([^)]+\))\s*(\d+(?:\.\d+)?%)?/g;
function angleToPoints(deg) {
...
export function parseLinearGradient(css) {
    ...
    for (const stop of stopText.match(COLOR_STOP_RE) ?? []) {
        const row = stop.match(/^(#[0-9A-Fa-f]{3,8}|rgba?\([^)]+\)|hsla?\([^)]+\))\s*(\d+(?:\.\d+)?%)?$/);
        if (!row)
            continue;
        colors.push(row[1]);
