...
export function useEnteringTurnIds(turnIds) {
    const seenTurnIds = useRef(new Set());
    const isInitialMount = useRef(true);
    const [enteringTurnIds, setEnteringTurnIds] = useState(() => new Set());
    useEffect(() => {
        const currentIds = turnIds.filter((id) => Boolean(id));
        ...
        const timer = setTimeout(() => setEnteringTurnIds(new Set()), WIDGET_TURN_ENTER_MS);
        return () => clearTimeout(timer);
    }, [turnIds]);
    return enteringTurnIds;
}
export function useWidgetSheetAnimation(visible) {
...
export function useTurnEnterAnimation(active, role) {
    const opacity = useRef(new Animated.Value(active ? 0 : 1)).current;
    const translateX = useRef(new Animated.Value(active && role === "user" ? 10 : active ? -8 : 0)).current;
...
export function useLauncherBubbleAnimation(open) {
...
export function useThinkingDotAnimation(delayMs) {
