import type { Turn, TurnUI } from "../types";
...
function mapUi(raw: Record<string, unknown> | null | undefined): TurnUI | null {
...
export function serverMessagesToTurns(
...
function turnContentKey(turn: Turn): string {
  if (turn.running) {
...
export function mergeServerTurns(
  local: Turn[],
  server: Turn[],
  welcomeMessage = "",
): Turn[] {
  ...
  if (!serverTurns.length) return local;
  ...
  const serverIdClaimed = new Set<string>();
  // Map from localId → matched server turn
  const localIdToServer = new Map<string, Turn>();
  ...
  const merged = serverTurns.map((turn) => {
    ...
    const matchedLocalId = [...localIdToServer.entries()].find(
      ([, srv]) => srv.id === turn.id,
    )?.[0];
    if (matchedLocalId) {
  ...
  const mergedKeys = new Set(
    merged.filter((turn) => !turn.running).map((turn) => turnContentKey(turn)),
  );

  for (const turn of local) {
    if (serverTurns.some((row) => row.id === turn.id)) continue;
    ...
    if (turn.localId && localIdToServer.has(turn.localId)) {
      ...
      if (mergedIds.has(matched.id)) continue;
    }
    ...
    if (isWelcomeTurn(turn)) {
      if (!merged.some(isWelcomeTurn)) merged.push(turn);
      continue;
    }
 ...
 */
export function signalChatClosed(
  tenantId: string | undefined,
  sessionId: string,
  options?: { agentUrl?: string; productId?: string; apiKey?: string },
): void {
  if (!options?.apiKey?.trim() || !sessionId) return;
...
export async function fetchSessionConversationMessages(
  tenantId: string | undefined,
  sessionId: string,
  options?: {
    agentUrl?: string;
  ...
  const res = await fetch(
    `${agentUrl}/products/${productId}/sessions/${encodeURIComponent(sessionId)}/conversation/messages?${qs}`,
    { headers },
  );
  if (!res.ok) {
  ...
  const data = (await res.json()) as SessionConversationMessagesResponse;
  return {
