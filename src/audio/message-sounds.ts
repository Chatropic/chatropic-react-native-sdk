...
function generateSentSamples(sampleRate: number): Float32Array {
  const dur = 0.1;
...
function generateReceivedSamples(sampleRate: number): Float32Array {
  const dur = 0.26;
  const n = Math.floor(sampleRate * dur);
  const out = new Float32Array(n);
  const note = (freq: number, start: number, vol: number) => {
    for (let i = 0; i < n; i++) {
...
const wavCache: Partial<Record<"sent" | "received", string>> = {};

async function getWavUri(type: "sent" | "received"): Promise<string> {
...
async function playSound(type: "sent" | "received"): Promise<void> {
