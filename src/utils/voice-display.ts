...
export function trimVoiceDisplayText(
  text: string,
  maxLines: number = VOICE_DISPLAY_MAX_LINES,
): string {
...
export function pickVoiceDisplayLine(
  transcript: VoiceTranscriptLine[],
  state: VoiceAgentState,
): VoiceTranscriptLine | null {
  ...
  if (state === "listening" || state === "connecting") {
    ...
    if (userLine && !userLine.final) return userLine;
    const agentLine = [...transcript]
      .reverse()
      .find((line) => line.role === "agent" && line.text.trim());
...
export const VOICE_STATUS_LABEL: Record<VoiceAgentState, string> = {
  idle: "Voice off",
...
export const VOICE_HINT: Record<VoiceAgentState, string> = {
  idle: "",
