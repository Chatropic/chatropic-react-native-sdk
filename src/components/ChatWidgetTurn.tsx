...
function AgentAvatar({ isDark }: { isDark: boolean }) {
...
export function ChatWidgetTurn({
  ...
  const enterAnimation = useTurnEnterAnimation(
    animate,
  ...
  return (
    <Animated.View style={[styles.agentRow, enterAnimation.style]}>
      ...
      {/* Bubble column */}
      <View style={[styles.bubbleColumn, hasWidget ? styles.fullWidth : undefined]}>
        {showBubble ? (
          <MessageBubble
            role="agent"
            text={turn.text ?? ""}
        ...
        {showThinking ? (
          <ThinkingBubble config={config} colorScheme={colorScheme} />
        ) : null}

        {hasWidget ? (
          <View style={[styles.widgetWrap, showBubble && styles.widgetWrapSpaced]}>
            {turn.ui!.component === "NavigationCard" ? (
              <NavigationCard
                label={String(turn.ui!.props.label ?? "")}
                path={String(turn.ui!.props.path ?? "")}
                requiresAuth={Boolean(turn.ui!.props.requiresAuth)}
                colorScheme={colorScheme}
                onNavigate={(path) => onNavigate?.(path)}
              />
            ) : canRenderProductWidget(turn.ui!.component, turn.ui!.props) ? (
              <ProductListWidget
                component={turn.ui!.component}
                props={turn.ui!.props}
                colorScheme={colorScheme}
                disabled={inputLocked}
                onAddToCart={(message, _item, displayMessage) =>
                  sendMessage(message, { displayText: displayMessage })
                }
              />
            ) : (
              <UnknownWidgetFallback
        ...
        {!turn.running && (showBubble || hasWidget) ? (
